...
const NAV_ITEMS = [
  {
    label: "Protocols",
    items: [
      { name: "Protocol Library", href: "/protocols", icon: "library" },
      { name: "Upload New", href: "/", icon: "upload" },
    ],
  },
  {
    label: "Tools",
  ...
  },
  {
    label: "Budget",
    items: [
      { name: "Site Budget Wizard", href: "/budget", icon: "dollar" },
    ],
  },
  {
    label: "Extract",
  ...
  // Quality Dashboard removed — not providing value in current state
  {
    label: "Manage",
    items: [
      { name: "Procedure Library", href: "/manage/procedures", icon: "database" },
      { name: "Feedback Tracker", href: "/feedback", icon: "feedback" },
    ],
  },
  {
    label: "",
...
const icons: Record<string, React.ReactNode> = {
  ...
  clipboard: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5A3.375 3.375 0 006.375 7.5H6M15.75 18.75h-6A2.25 2.25 0 017.5 16.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M12 2.25c.621 0 1.125.504 1.125 1.125v1.5c0 .621-.504 1.125-1.125 1.125m0-3.75h3.75m-3.75 0H8.25" />
    </svg>
  ),
  database: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-3">
        {NAV_ITEMS.map((group) => (
          <div key={group.label} className={cn("mb-4", group.label === "Manage" && "mt-auto pt-3 border-t border-neutral-200")}>
            {!collapsed && (
